...
import com.bjxls.classfile.ClassFile;
import com.bjxls.rtda.Slots;

/**
 * @author BJXLS
 * 
 * @description: 如何设法保证同一个对象的 class == 返回 true?如果不自己定义 classloader，那么由系统提供的统一的
...
public class Zclass {
    ...
    public Zmethod getMethod(String name, String @description) {
        for (Zclass clazz = this; clazz != null; clazz = clazz.superClass) {
            for (Zmethod method : methods) {
                if (method.name.equals(name) && method.descriptor.equals(@description)) {
                    return method;
                }
            }
        }
